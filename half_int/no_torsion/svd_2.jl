function define_matrix_for_SVD_2_vec(state_8p::Array{Complex{Float64},1})
    ...
    #for u3 in 1:1, Keff3 in 1:RangeK[u3] #special line for k=2

        #(x3,y3) = S[u3,Keff3,:]

        if configs_8p_ind1_svd[x3,y3] + configs_8p_ind2_svd[x3,y3] > 0
            ...
            for index1 in 1:configs_8p_ind1_svd[x3,y3]
                ...
                for index2 in 1:configs_8p_ind2_svd[x3,y3]
                    ...
                    (x2,U1,i1,U2,X1,X2,Fold) = translate_8p_SVD_ind2_vec_to_ind(index2,x3,y3)
                    ...
                    U3 = Uf[i3,i3,k3,x3,y3]

                    index_old = translate_8p_ind_to_vec_bSVD(x2,U1,i1,U2,X1,i3,U3,X2,Fold)
            ...
            while count < 1
                ...
                if abs(mapU[x3,y3,temp_index1]) > TOLERANCE
                    ...
                    (i3,k3) = SF[x3,y3,temp_index1,:]

                    temp_phase = conj(mapU[x3,y3,temp_index1] /
                    abs(mapU[x3,y3,temp_index1])) #*
                    #phase_factor_emb_full(u3,x3,y3,1,i3,i3,1,k3,k3)
...
function define_6p_state_vec(new_ten::Array{Complex{Float64},3},emb::Array{Complex{Float64},3})
    ...
    for index in 1:configs_6p
        ...
        (x2,U1,U2,U3,F) = translate_6p_vec_to_ind(index)

        (i5,M1,N1) = SU[x2,x2,U1,:]

        (x1,M5,N5) = SU[M1,N1,U2,:]

        (x3,M3,N3) = SU[M5,N5,U3,:]

        (k6,x4) = SF[M3,N3,F,:]

        for Fold in 1:RangeF[x1,x1]

            (i1,k1) = SF[x1,x1,Fold,:]

            Z1 = Uf[i1,i1,k1,x1,x1]

            #E1p = Ef[x1,x1,1]

            X1 = Xf[M1,N1,x1,x1,M5,N5]

            #K1 = Kf[1,x1,x1]

            #K3 = Kf[1,x3,x3]

            #T1 = Uf[x1,x1,i1,k1,k1]

            #E1 = Ef[k1,k1,1]

            X2 = Xf[M5,N5,x3,x3,M3,N3]

            index1_old = Fold

            index2_old = translate_8p_SVD_ind2_ind_to_vec(x2,U1,i1,Z1,X1,X2,F,x3,x3)

            Amp_new[index] +=
                new_ten[x3,x3,index2_old] *
                emb[x1,x1,index1_old] *
                v(x1)^2 * v(x3)^2
