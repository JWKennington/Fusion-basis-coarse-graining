...
function compute_univ_index_max()
    ...
    for a1 in 1:k+1, b1 in 1:k+1
        ...
        for i in 1:k+1, j in 1:k+1, a2 in 1:k+1, b2 in 1:k+1

            if coupling_rules(a1,i,a2) == 1 &&
                coupling_rules(b1,j,b2) == 1
...
function compute_univ_index()
    ...
    SU = zeros(Int,k+1,k+1,maxU,4)
    Uf = zeros(Int,k+1,k+1,k+1,k+1,k+1,k+1)
...
function Index_for_two_reps()

    RangeP = zeros(Int)
    SP = zeros(Int,(k+1)^2,2)
    Pf = zeros(Int,k+1,k+1,1)
...
const (RangeP,SP,Pf) = Index_for_two_reps();
